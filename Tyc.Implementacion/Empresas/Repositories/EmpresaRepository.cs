...
public class EmpresaRepository : IEmpresaRepository
{
    ...
    public Empresa Update(TycBaseContext context, Empresa entity)
    {
        ...
        existing.ManejaTerminosPersonaJuridica = entity.ManejaTerminosPersonaJuridica;
    ...
    public async Task<Empresa> GetByIdAsync(TycBaseContext context, int id)
    {
        return await Task.Run(() => context.GetTable<Empresa>()
            .FirstOrDefault(x => x.EmpresaId == id));
    }

    public async Task<Empresa> CreateAsync(TycBaseContext context, Empresa entity)
    {
        context.GetTable<Empresa>().InsertOnSubmit(entity);
    ...
    public async Task<Empresa> UpdateAsync(TycBaseContext context, Empresa entity)
    {
        var existing = await Task.Run(() => context.GetTable<Empresa>()
            .FirstOrDefault(x => x.EmpresaId == entity.EmpresaId));
    ...
    {
        return await Task.Run(() => context.GetTable<Empresa>().Any(x => x.EmpresaId == id));
    }

    public async Task<bool> ExisteSubdominioAsync(TycBaseContext context, string subdominio, int? excludeId = null)
    {
        var query = context.GetTable<Empresa>()
            .Where(x => x.Subdominio == subdominio);
